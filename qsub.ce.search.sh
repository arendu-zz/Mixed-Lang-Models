...
set -e
if [ $# -ne 8 ]; then
  echo 1>&2 "Usage: $0 L1_DATA_NAME MODEL_SIZE NUM_LAYER CONTEXT_ENCODER L2_DATA_NAME SEED RANK LSR"
  exit 3
fi
...
INIT_MODEL_DIR=es.0.ce.$CONTEXT_ENCODER.la.$LOSS_AT.seed.2000.es.$EMBEDDING_SIZE.ep.$EMBEDDING_PRETRAIN.rs.$MODEL_SIZE.nl.$NUM_LAYERS.ca.$CHARAWARE
