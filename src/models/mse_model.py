...
class L2_MSE_CLOZE(nn.Module):
    ...
    def forward(self, batch):
        ...
        while continue_iter:
            it += 1
            hidden = self.get_hiddens(mixed_encoded, lengths, batch_size)
...
class MSE_CLOZE(nn.Module):
    def __init__(self,
        ...
        self.rnn = nn.LSTM(self.input_size, self.rnn_size,
                           ...
                           batch_first=True,
                           bidirectional=True)
        self.highway_ff = nn.Sequential(HighwayLayer((2 * rnn_size) + (1 * self.input_size)),
                                        nn.Linear((2 * self.rnn_size + (1 * self.input_size)), self.input_size))
        self.train_mode = train_mode
        self.tanh = torch.nn.Tanh()
        self.z = torch.zeros(1, 1, self.rnn_size, requires_grad=False)
    ...
    def get_ortho_representations(self, l1_data):
        if self.train_mode == 2:
            ...
            l1_encoded = self.encoder(l1_data)
            l1_rand_encoded = torch.zeros_like(l1_encoded).type_as(l1_encoded)
    ...
    def forward(self, batch, get_acc=True):
        ...
        rand = torch.zeros_like(encoded[word_mask == 1, :]).uniform_(-1.0, 1.0)  # replace with rand vector
        rand.requires_grad = False
        encoded[word_mask == 1, :] = rand
        hidden = self.get_hiddens(encoded, lengths, batch_size)
        hidden_ortho = self.get_ortho_representations(l1_data)
        hidden = torch.cat((hidden_ortho, hidden), dim=2)
...
#class MSE_ORTHOGRAPHIC_CLOZE(nn.Module):
#    def __init__(self,
#                 input_size,
#                 rnn_size,
#                 encoder,
...
#        super().__init__()
#        self.encoder = encoder
#        #self.categorical_sampler = torch.distributions.Categorical(probs=torch.tensor([1.0, 1.0, 1.0]))
#
#        for n, p in self.encoder.named_parameters():
#            p.requires_grad = False
#
#        self.nn_mapper = nn_mapper
#        if self.nn_mapper is not None:
#            for n, p in self.nn_mapper.named_parameters():
#                p.requires_grad = False
#
#        self.dropout = torch.nn.Dropout(dropout)
#        self.input_size = input_size
...
#                                        HighwayLayer((2 * rnn_size) + (1 * self.input_size)),
#                                        nn.Dropout(dropout),
#                                        nn.Linear((2 * self.rnn_size + (1 * self.input_size)), self.input_size))
#        self.tanh = torch.nn.Tanh()
#        self.z = torch.zeros(1, 1, self.rnn_size, requires_grad=False)
...
#        rand.requires_grad = False
#        encoded[word_mask == 1, :] = rand
#        hidden_context = self.get_hiddens(encoded, lengths, batch_size)
#        hidden_ortho = self.get_ortho_representations(l1_data)
#        hidden = torch.cat((hidden_ortho, hidden_context), dim=2)
#        out = self.tanh(self.highway_ff(hidden))
#        out = out[l1_idxs == 1, :] #ignore other outputs for loss (basically ignores <pad>)
#        loss = self.get_loss(out, l1_encoded[l1_idxs == 1, :])
#        if get_acc:
#            acc = self.get_acc(out, self.encoder.weight.data, l1_data[l1_idxs == 1])
#        else:
...
#
#
#class L2_MSE_ORTHOGRAPHIC_CLOZE(nn.Module):
#    def __init__(self,
#                 encoder,
#                 rnn,
#                 highway_ff,
...
#        while continue_iter:
#            it += 1
#            hidden_context = self.get_hiddens(mixed_encoded, lengths, batch_size)
#            hidden = torch.cat((mixed_encoded, hidden_context), dim=2)
#            out = self.tanh(self.highway_ff(hidden))
#            mixed_encoded[l2_idxs == 1, :] = out[l2_idxs == 1, :]  # replace all l2_idxs with predictions...
#            continue_iter = it <= self.iters
#        if l2_idxs.nonzero().numel() > 0:
